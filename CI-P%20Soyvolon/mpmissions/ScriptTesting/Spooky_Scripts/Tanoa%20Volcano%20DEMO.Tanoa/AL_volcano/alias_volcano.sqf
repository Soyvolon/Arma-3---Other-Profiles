...
params ["_alias_delay","_crater","_lightning","_lava","_lethal","_volcano_protect"];

protect_volcano =_volcano_protect; publicVariable "protect_volcano";
_alias_radius = 200;
_alias_volcano = "Sign_Sphere200cm_F" createVehicle [9940.07,12104.8,30]; _alias_volcano hideObjectGlobal true;

alias_eruption = false; publicVariable "alias_eruption";
volcano=true;publicVariable "volcano";

[[_alias_volcano, _alias_radius,_alias_delay,_lethal],"AL_volcano\alias_volcano_effect.sqf"] remoteExec ["execVM",0,true];
if (_crater) then {[[_alias_volcano, _alias_radius,_alias_delay],"AL_volcano\crater_SFX.sqf"] remoteExec ["execVM",0,true]};
//if (_lava) then {[[_alias_volcano],"AL_volcano\lava_flow.sqf"] remoteExec ["execVM",0,true]};
...
if (_alias_delay>0) then 
{
	...
	while {volcano} do 
	{
		...
		sleep 1;
		switch (shape_tip) do {
			case "schije": {[[_alias_volcano],"AL_volcano\alias_schije.sqf"] remoteExec ["execVM"]};
			case "puf": {[[_alias_volcano],"AL_volcano\alias_blow.sqf"] remoteExec ["execVM"]};
			case "scantei": {[[_alias_volcano],"AL_volcano\alias_scantei.sqf"] remoteExec ["execVM"]};
		};			
		//_alias_volcano setVariable ["erupt",nil,true];
		sleep _alias_delay;
	};
