...
["ace_interact_menu_newControllableObject", {
	...
	_insertChildren = {
		...
		private _actions = [];
		{
			...
			diag_log text (["[SVLN]", "[RADAR JAMMER]", "TRACE:", "Action dump - action loader", _actions select _x] joinString " ");

		} forEach [0, 1, 2];

		diag_log text (["[SVLN]", "[RADAR JAMMER]", "TRACE:", "Action dump", _actions] joinString " ");
