...
_startingAltitude = (getPosASL _positionA) select 2;

_coordinatesA = getPosASL _positionA;
...
_x = ((getPosASL _positionB select 0)-(getPosASL _positionA select 0));
_y = ((getPosASL _positionB select 1)-(getPosASL _positionA select 1));
...
for [{_i=0},{_i<_dis},{_i=_i+_step}] do {
	_x = sin(_dir)*_i;
	_y = cos(_dir)*_i;
	_obj setPosASL [(getPosASL _positionA select 0) + _x,(getPosASL _positionA select 1) + _y, _startingAltitude];
	sleep _timg;
};

// _this attachTo [left_mover, [-1, -8, 2]];
// _this attachTo [seg_1, [0, -16, 0]];